...
import {
  loadAmenities, saveAmenities, DEFAULT_AMENITIES,
  loadUserAmenitiesConfig, saveUserAmenitiesConfig,
} from "../firebase.js";
...
export default function AmenitiesConfig({ onBack, isAdmin, user }) {
  const uid = user?.uid || "";
  ...
  const [hasUserCfg, setHasUserCfg] = useState(false);

  useEffect(() => {
    const load = async () => {
      const shared = await loadAmenities();
      const master = shared.length ? shared : DEFAULT_AMENITIES;
      setAdminItems(master);
      if (isAdmin) {
        setItems(master);
      } else {
        const userCfg = await loadUserAmenitiesConfig(uid);
        if (userCfg && userCfg.length > 0) {
          setItems(userCfg);
          setHasUserCfg(true);
        } else {
          setItems(master.map(i => ({ ...i })));
          setHasUserCfg(false);
        }
      }
      setLoading(false);
  ...
  const persist = async (updated) => {
    ...
    if (isAdmin) {
      ...
      await saveUserAmenitiesConfig(uid, updated);
      setHasUserCfg(true);
    }
    setSaving(false);
  };
  ...
  const resetToAdmin = async () => {
    if (!window.confirm("Reset to admin defaults? Your personalised config will be replaced.")) return;
    const copy = adminItems.map(i => ({ ...i }));
    setItems(copy);
    setSaving(true);
    await saveUserAmenitiesConfig(uid, copy);
    setHasUserCfg(false);
    setSaving(false);
  };
    ...
    <>
      ...
      <div style={{ minHeight:"100vh", background:"#f8fafc", fontFamily:"'Inter',sans-serif" }}>
        ...
        <div style={{ background:"#fff", borderBottom:"1px solid #e2e8f0", padding:"10px 16px", display:"flex", alignItems:"center", gap:12, position:"sticky", top:0, zIndex:100, boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
          ...
          {saving && <span style={{ color:"#94a3b8", fontSize:11, background:"#f1f5f9", borderRadius:20, padding:"2px 10px", whiteSpace:"nowrap" }}>Saving...</span>}
          {!isAdmin && hasUserCfg && (
            <button onClick={resetToAdmin}
              style={{ background:"#fef3c7", border:"1px solid #fde68a", borderRadius:8, padding:"7px 12px", color:"#92400e", cursor:"pointer", fontSize:12, fontWeight:600, whiteSpace:"nowrap" }}>
              ↺ Reset to Defaults
            </button>
          )}
        </div>
        ...
        <div style={{ maxWidth:720, margin:"0 auto", padding:"20px 16px" }}>
          ...
          <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:16, flexWrap:"wrap", gap:16 }}>
            <div>
              ...
              <p style={{ color:"#64748b", fontSize:13, margin:0 }}>
                {items.length} feature{items.length !== 1 ? "s" : ""} · Max possible score: <strong style={{ color:"#0f172a" }}>{totalMax}</strong>
              </p>
            </div>
          ...
          {/* Context banner */}
          <div style={{
            background: isAdmin ? "#eff6ff" : hasUserCfg ? "#f5f3ff" : "#f0fdf4",
            border: `1px solid ${isAdmin ? "#bfdbfe" : hasUserCfg ? "#ddd6fe" : "#bbf7d0"}`,
            borderRadius:10, padding:"11px 16px", marginBottom:20, display:"flex", gap:10, alignItems:"center",
          }}>
            <span style={{ fontSize:16 }}>{isAdmin ? "🛡" : hasUserCfg ? "✏️" : "📋"}</span>
            <span style={{ fontSize:13, color: isAdmin ? "#1e40af" : hasUserCfg ? "#4c1d95" : "#14532d", lineHeight:1.5 }}>
              {isAdmin
                ? <><strong>Admin view</strong> — changes here update the shared default list all users start from.</>
                : hasUserCfg
                  ? <><strong>Your personalised config</strong> — you've customised this list. Use "↺ Reset to Defaults" to restore admin defaults.</>
                  : <><strong>Loaded from admin defaults</strong> — make any change to save your own personalised copy.</>
              }
            </span>
          </div>
          ...
          {!loading && items.length === 0 && (
            <div style={{ textAlign:"center", padding:"60px 24px", background:"#fff", borderRadius:16, border:"1px solid #e2e8f0" }}>
              ...
              <div style={{ fontFamily:"'Playfair Display',serif", fontSize:20, color:"#0f172a", marginBottom:8 }}>No features yet</div>
              <div style={{ color:"#94a3b8", fontSize:13, marginBottom:24 }}>Add amenities like Solar, Shed, Backyard and assign score points.</div>
              <button onClick={() => { setEditing(null); setShowModal(true); }}
          ...
          {!loading && items.length > 0 && (
            <div style={{ background:"#fff", border:"1px solid #e2e8f0", borderRadius:14, overflow:"hidden", boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 180px 80px", background:"#f8fafc", borderBottom:"2px solid #e2e8f0", padding:"10px 20px" }}>
                ...
                <span style={{ fontSize:11, fontWeight:700, color:"#94a3b8", textTransform:"uppercase", letterSpacing:1 }}>Actions</span>
              </div>
              {items.map((item, idx) => (
      ...
      <div style={{ background:"#fff", borderRadius:16, padding:32, width:"min(440px,95vw)", boxShadow:"0 20px 60px rgba(0,0,0,0.15)" }}>
        ...
        <div style={{ marginBottom:18 }}>
          ...
          <input value={name} onChange={e => { setName(e.target.value); setError(""); }}
            ...
            autoFocus style={inp} onKeyDown={e => e.key === "Enter" && handleSave()} />
        </div>
        ...
        {error && <div style={{ background:"#fef2f2", border:"1px solid #fecaca", borderRadius:8, padding:"9px 14px", marginBottom:16, color:"#dc2626", fontSize:13 }}>{error}</div>}
        <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}>
          <button onClick={onClose} style={{ background:"transparent", border:"1px solid #e2e8f0", borderRadius:8, padding:"10px 20px", color:"#64748b", cursor:"pointer", fontSize:14 }}>Cancel</button>
          <button onClick={handleSave} style={{ background:"#8b5cf6", border:"none", borderRadius:8, padding:"10px 24px", color:"#fff", cursor:"pointer", fontSize:14, fontWeight:600 }}>
